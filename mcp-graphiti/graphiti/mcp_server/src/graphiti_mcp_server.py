...
from services.factories import DatabaseDriverFactory, EmbedderFactory, LLMClientFactory, RerankerFactory
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            try:
                ...
                # Create reranker client if using OpenAI (or other supported providers)
                try:
                    reranker_client = RerankerFactory.create(self.config.llm, llm_client)
            ...
            if reranker_client:
                logger.info('Using Reranker client for search optimization')
            else:
                logger.info('No Reranker client configured')
