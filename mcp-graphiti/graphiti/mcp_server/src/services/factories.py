...
class RerankerFactory:
    ...
    @staticmethod
    def create(config: LLMConfig, llm_client: LLMClient | None = None) -> CrossEncoderClient:
        """Create a Reranker client based on the configured LLM provider."""
        import logging
        ...
        match provider:
            case 'openai':
                ...
                api_key = config.providers.openai.api_key
                # We don't strictly validate for dummy keys here if already validated for LLM
                # But _validate_api_key handles it fine if needed.
                # Actually, let's just reuse the llm_client if provided and compatible

                if llm_client and isinstance(llm_client, OpenAIClient):
                    logger.info('Creating OpenAI Reranker client by reusing LLM client')
                    return OpenAIRerankerClient(client=llm_client)
                ...
                logger.info('Creating new OpenAI Reranker client')
                return OpenAIRerankerClient(config=llm_config)

            case _:
                # For other providers, we might not have a dedicated reranker yet.
                # Graphiti-core might default to OpenAI if we return None, so we should
                # probably provide a default or the LLM client if possible.
                if llm_client and isinstance(llm_client, (OpenAIClient, GeminiClient, AnthropicClient)):
                     # OpenAIRerankerClient can theoretically take any client that has a chat.completions.create method
                     # though it's risky for non-openai ones due to logprobs/logit_bias.
                     pass

                # If no specific reranker is available, let return None and handle in service
                return None
